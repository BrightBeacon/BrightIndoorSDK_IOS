...
#define STARTPOINT CGPointMake(30, [UIScreen mainScreen].bounds.size.height-30)
...
- (id)initWithFrame:(CGRect)frame menus:(NSArray *)aMenusArray
{
    ...
    if (self) {
        ...
        for (int i = 0; i < count; i ++)
        {
            ...
            if (count<=5){
            NSInteger end = ENDRADIUS;
                item.endPoint = CGPointMake(STARTPOINT.x + end * sinf(i * M_PI_2 / (count - 1)), STARTPOINT.y - end * cosf(i * M_PI_2 / (count - 1)));
                item.nearPoint = CGPointMake(STARTPOINT.x + (end-10) * sinf(i * M_PI_2 / (count - 1)), STARTPOINT.y - (end-10) * cosf(i * M_PI_2 / (count - 1)));
                item.farPoint = CGPointMake(STARTPOINT.x + (end+20) * sinf(i * M_PI_2 / (count - 1)), STARTPOINT.y - (end+20) * cosf(i * M_PI_2 / (count - 1)));
            }else{
                NSInteger end = i<5?ENDRADIUS:(ENDRADIUS/2.0);
                NSInteger sum = i<5?5:count-5;
                NSInteger j = i<5?i:i-5;
                item.endPoint = CGPointMake(STARTPOINT.x + end * sinf(j * M_PI_2 / (sum - 1)), STARTPOINT.y - end * cosf(j * M_PI_2 / (sum - 1)));
                item.nearPoint = CGPointMake(STARTPOINT.x + (end-10) * sinf(j * M_PI_2 / (sum - 1)), STARTPOINT.y - (end-10) * cosf(j * M_PI_2 / (sum - 1)));
                item.farPoint = CGPointMake(STARTPOINT.x + (end+20) * sinf(j * M_PI_2 / (sum - 1)), STARTPOINT.y - (end+20) * cosf(j * M_PI_2 / (sum - 1)));
            }
        ...
        // add the "Add" Button.
        _addButton = [[QuadCurveMenuItem alloc] initWithImage:[UIImage imageNamed:@"bg-addbutton"]
                                       highlightedImage:[UIImage imageNamed:@"bg-addbutton"]
                                           ContentImage:nil
...
- (void)quadCurveMenuItemTouchesEnd:(QuadCurveMenuItem *)item
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        ...
        self.bgimagview.frame = CGRectMake(0, self.frame.size.height, 0, 0);
    }];
...
- (void)_expand
{
    ...
    if (_flag == 0) {
        [UIView animateWithDuration:.3 animations:^{
            self.bgimagview.frame = CGRectMake(0, self.frame.size.height-160, 160, 160);
        }];
    }
